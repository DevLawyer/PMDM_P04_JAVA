 ...
 *
 *
 * Class Employee: This class is going to be uninstanciable, hence, it is going
 * to be abstract. It is also going to implement the interface Serializable.
 * Both classes Programmer and Analyst will extend this class, so they will both
 * be serializable. It will have 2 constructors: one with a parameter for every
 * attribute this class has, and another one with a parameter for each attribute
 * except maxSalary. This class will use MyExceptions for exception control.
 *
 * Employee will contain the following methods: -2 Constructors. -Getters and
 * Setters for each attribute. -Abstract method toString(). -Abstract method
 * toBinary(). -Abstract method parseEmployee(String). -Abstract method
 * udpateSalary().
 *
...
public abstract class Employee implements Serializable {
    ...
    public static Employee parseEmployee(String s) {
        ...
        switch (data[0]) {
            case "Analyst" ->
                res = new Analyst(data[1], data[2], data[3], data[4], data[5],
                        data[6], data[7]);
            case "Programmer" ->
                res = new Programmer(data[1], data[2], data[3], data[4], data[5],
                        data[6], data[7]);
            default -> {
                res = null;
            }
